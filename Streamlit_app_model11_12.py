...
with modelFeatureSelection:
    ...
    pH = st.sidebar.selectbox("pH column name", columns)
    time = st.sidebar.selectbox("Time column name", columns)
    y = st.sidebar.selectbox("Y variable column name", columns)
    
    Abase = st.sidebar.number_input("Abase", value = 11) 
    Eabase = st.sidebar.number_input("Eabase", value = -10000) 
    Cbase = st.sidebar.number_input("Cbase", value = 2)
    Aacid = st.sidebar.number_input("Aacid", value = 10) 
    Eaacid = st.sidebar.number_input("Eaacid", value = -10000)
    Cacid = st.sidebar.number_input("Cacid", value = -2)
    
    x0 = np.array([Abase, Eabase, Cbase, Aacid, Eaacid, Cacid])
    x0 = x0.astype(float)
    
    model_function_list = [model11_calc, model12_calc]
    images_list = [["Acid/Base Catalysed Linear","model11_calc_pic.png"], 
                    ["Acid/Base Catalysed First Order", "model12_calc_pic.png"]]
...
def optimising_parameters(model_selector):
    optimised_parameters = minimize(model_error_squared, x0, args=(df[y], model_selector, df[temperature], df[time], df[pH]), method='Nelder-Mead', jac=None, options={'maxiter': 99999, 'maxfev': None, 'disp': False, 'return_all': False, 'initial_simplex': None, 'xatol': 0.000001, 'fatol': 0.000001, 'adaptive': False})
...
with modelTraining:
    ...
    y_predicted = model_selector(optimising_parameters(model_selector), df[temperature], df[time], df[pH])
    ...
    st.write({"Abase":optimising_parameters(model_selector)[0], "Eabase":optimising_parameters(model_selector)[1], "Cbase":optimising_parameters(model_selector)[2], "Aacid":optimising_parameters(model_selector)[3], "Eaacid":optimising_parameters(model_selector)[4], "Cacid":optimising_parameters(model_selector)[5]})
    ...
    Model_mapping = {model11_calc: "Acid/Base Catalysed Linear", 
                     model12_calc: "Acid/Base Catalysed First Order"}
    ...
    if modelForLoopSelection == "Yes":
        ...
        for model in selected_models:
            x = actual_vs_predicted_metrics(model(optimising_parameters(model),df[temperature], df[time], df[pH]), df[y])
...
with residuals:
    ...
    residual_df = pd.DataFrame({"Temperature":df[temperature], "Time":df[time], "pH":df[pH], "y actual":df[y], "y predicted": y_predicted, "Residuals":df[y]-y_predicted, "Normalised Residuals": y_predicted/df[y]})
